...
Lemma interp_unCA i e t r :
  interp i (e :: t ++ r) = interp i (t ++ e :: r).
Proof.
by rewrite interp_cons 2!interp_cat interp_cons unCA.
Qed.
...
Lemma ptr_split i x t :
  x \in ptrs t ->
  def (interp i t) ->
  interp i t = (plook' i x :-> Dyn.val (pread' x t)) :+ interp i (pfree x t).
Proof.
move=>H D.
rewrite -ptr_has in H.
rewrite (heap_etaP (h:=interp i t) (x:=plook' i x)).
rewrite -(freeR D H).
apply: congeqUh.
set lf := (look _ _).
set rt := (pread' _ _).
suff H' : lf = rt; first by rewrite H'.
rewrite /lf /rt lookR //.
by apply: domR.
Qed.

Lemma cong_pts (i : ctx) (t t' r : synheap) (x : nat) (d : dynamic) :
  x \in ptrs t ->
  interp i (t' ++ r) = interp i (pfree x t) ->
  d = pread' x t ->
  def (interp i (Pts x d :: t' ++ r)) ->
  def (interp i t) ->
  interp i (Pts x d :: t' ++ r) = interp i t.
Proof.
move=>x_in_t rec H D1 D2.
rewrite (ptr_split x_in_t D2).
rewrite -rec.
rewrite interp_cons.
apply: congeqUh.
rewrite [einterp _ _]/=.
rewrite /plook /=.
rewrite /= in D1.
case: (onth _ _) D1=>[x'|]; last by case: (_ ++ _).
by rewrite -H.
Qed.

Lemma free_heap i h t :
  def (interp i t) ->
  h \in vars t ->
  interp i t = interp i [:: Var h] :+ interp i (hfree h t).
Proof.
move=>D H.
rewrite -has_pred1 has_count in H.
case H: (count _ _) H=>[//|[|n]] _; last first.
  have H3: count (pred1 h) (vars t) > 1 by rewrite H.
  move: (countN_varfree H3 D)=>/= H4.
  by rewrite (empty_hfree _ H4) H4 un0h.
move/(interp_perm i): (count1_hfree H)=>H6.
by rewrite -H6 interp_cons.
Qed.
